...
foreach ($list as $elem){
    $nuevoElem["roles"] = "";
    $data["id"] = $elem->getId();
    $roles = $objControl->buscarRoles($data);

    foreach($roles as $rol){
        $nuevoElem["roles"] .= '<span class="badge rounded-pill text-bg-primary mx-1" id="'. $elem->getId() .'-'. $rol->getObjRol()->getId() .'">
        '. $rol->getObjRol()->getRolDescripcion() . "</span>";
    }
